...
#### 3.变量API_TOKEN,可选，设置成与上面DASH_TOKEN相同的值，作用是支持客户端设置节点名称

这个API_TOKEN功能，需要配合玩具脚本，在客户端设置SUB_NAME为节点名称，则哪吒面板会自动生成相应节点名称。需要等待一会。

特别提醒: 开启这个功能后，不支持在哪吒后台面板改名，节点名称只能由客户端设置
